...
def tokens_to_string(tokens: List[int]) -> str:
    ...
    sound_end = "<|sound_end|>"
    strings = ''.join(f'<|sound_{token:04d}|>' for token in tokens)
    return f"{sound_start}{strings}{sound_end}" 
...
def process_and_save_text(
    ...
    batch_conversation = subset["conversations"] # List of conversations
    ...
    batch = []
    for conversation, index in zip(batch_conversation, batch_index):
        token_conversation = []
            ...
            try:
                ...
                for turn in conversation:
                    if turn["role"] == "assistant":
                        token_conversation.append(turn)
                        continue
                    text = turn["content"].replace("\n", " ")
                    # print(text)
                    role = turn["role"]
                    audio_tokens = tts_processor.convert_text_to_tokens(text)[0]
                    # convert tensor to list[int]
                    audio_tokens = audio_tokens.cpu().tolist()
                    assert isinstance(audio_tokens[0], int)
                    token_conversation.append({
                        "content": tokens_to_string(audio_tokens),
                        "role": role
                    })
                batch.append(
                    {
                        "index": index,
                        "conversations": token_conversation,
                    }
                )
