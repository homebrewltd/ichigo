...
def process_and_save_text(
    ...
    logging.info(f"Process {process_id} will process {len(subset)} examples.")
    ...
    logger.info(f"Process {process_id} will save to {csv_file_path}.")
    ...
    logger.info(
        f"Process {process_id} will save failed indices to {saved_failed_indice_path}."
    )

    batch = []
    for text, index in zip(batch_prompt, batch_index):
        logger.info(f"Process {process_id} processing item sample {index}.")
...
def run_pipeline(
    ...
    logger.info(f"Total items in chunks: {total_items}")
    ...
    while any(p.is_alive() for p in worker_processes):
        ...
        logger.info(f"Processed: {processed_count.value}")
        time.sleep(60)
    ...
    logger.info(f"Final processed count: {processed_count.value}")
